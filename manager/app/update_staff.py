...
LEFT JOIN person_group using(group_id)
LEFT JOIN point using(point_id)
WHERE point_id={point_id};'''
    ...
    if not result:
        print(f'No staff for point_id: {point_id}')
        db.close()
        exit(0)
...
WHERE point_id={point_id};'''
    ...
    if result1:
        ...
        e_param = result1[3]
    if not entry_point:
        print(f'No embedding entry point for point_id: {point_id}')
        db.close()
        exit(0)
    ...
    print(f'Staff of point_id {point_id} updated {res}/{len(output)}')
