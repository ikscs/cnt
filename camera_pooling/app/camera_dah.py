...
class Camera():
            ...
            if len(rec) == 1:
                ...
                if rec[0].startswith('items['):
                    ...
                    if len(self.items) == idx:
                        ...
                        if mode in ('recognition', 'detection2'):
    ...
    def get_dt(self):
        start_dt = '2025-05-26 00:00:00'
        end_dt = '2025-05-27 00:00:00'
    ...
    def get_detection2(self, start_dt, end_dt):
        ...
        if return_code == 200:
            params = f"{self.params}&count={self.max_results}"
            return_code = self.query(f"mediaFileFind.cgi?action=findNextFile&{params}", 'detection2')
...
if __name__ == "__main__":
    ...
    res = camera.get_detection2(start_dt, end_dt)

    camera.close_factory()

    for e in camera.items:
        content = camera.get_media(e['summary']['file_path'])

        image_name = e['summary']['file_path'].split('=')[-1]
