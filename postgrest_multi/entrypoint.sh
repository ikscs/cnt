...
for api in "${INSTANCES[@]}"; do
  ...
  DB_URI="$db_uri" DB_SCHEMAS="$schemas" DB_ANON_ROLE="$anon_role" JWT_ROLE_CLAIM_KEY="$jwt_role_claim_key" JWT_SECRET="$jwt_secret" DB_PRE_REQUEST="$db_pre_request" SERVER_PORT="$port" envsubst < /etc/postgrest/api.template.conf > "$conf_file"
